    ...
    for label, value in labels.items():
        ...
        if type not in allowed_record_types:
            if type != "enabled":  # skip known non-record labels silently
                logger.warning(
                    f"[record_builder] Skipping unknown or unsupported record type '{type}' from label '{label}'"
                )
                continue
    ...
    if "A" in base_label_pairs:
        if "name" in base_label_pairs["A"]:
            ...
            if "value" in base_label_pairs["A"]:
                ...
                logger.warning(
                    f"[record_builder] {prefix}.A.name={base_label_pairs['A']['name']} label found with no matching {prefix}.A.value pair. Using configured host IP {value} as default."
                )
            try:
                force = _get_force(
                    labels=labels,
                    container_force_label=container_force_label,
                    record_force_label=f"{prefix}.A.force",
                )
        ...
        if "name" in base_label_pairs["CNAME"] and "value" in base_label_pairs["CNAME"]:
            ...
            try:
                force = _get_force(
                    labels=labels,
                    container_force_label=container_force_label,
                    record_force_label=f"{prefix}.CNAME.force",
                )
                record_intents.append(
                    RecordIntent(
                        container_id=container_id,
        ...
        for alias, pair in aliased_label_pairs["A"].items():
            if "name" in pair:
                ...
                if "value" in pair:
                    ...
                    value = settings.host_ip
                    logger.warning(
                        f"[record_builder] {prefix}.A.name={pair['name']} label found with no matching {prefix}.A.value pair. Using configured host IP {value} as default."
                    )
                try:
                    force = _get_force(
                        labels=labels,
                        container_force_label=container_force_label,
                        record_force_label=f"{prefix}.A.{alias}.force",
                    )
                    record_intents.append(
                        RecordIntent(
                            container_id=container_id,
            ...
            if "name" in pair and "value" in pair:
                ...
                try:
                    force = _get_force(
                        labels=labels,
                        container_force_label=container_force_label,
                        record_force_label=f"{prefix}.CNAME.{alias}.force",
                    )
                    record_intents.append(
                        RecordIntent(
                            container_id=container_id,
