...
def validate_record(
    new_record_intent: RecordIntent, existing_record_intents: Iterable[RecordIntent]
) -> None:
    """
    ...
    if isinstance(new_record, ARecord):
        if cname_records:
            raise RecordValidationError(
                f"{new_record.name} -> {new_record.value} - cannot add an A record when a CNAME record exists with the same name"
            )
    elif isinstance(new_record, CNAMERecord):
        if a_records:
            raise RecordValidationError(
                f"{new_record.name} -> {new_record.value} - cannot add a CNAME record when an A record exists with the same name"
            )
    else:
    ...
    if isinstance(new_record, CNAMERecord):
        if cname_records:
            raise RecordValidationError(
                f"{new_record.name} -> {new_record.value} - cannot have multiple CNAME records with the same name"
            )
    ...
    if isinstance(new_record, ARecord):
        ...
        if same_ip_records:
            raise RecordValidationError(
                f"{new_record.name} -> {new_record.value} - existing A record(s) detected with the same name and value"
            )
        ...
        for r in existing_records:
            if isinstance(r, CNAMERecord):
                if r.name in forward_map:
                    logger.warning(
                        f"Duplicate CNAME definitions detected in remote registry for domain {r.name}"
                    )
                    continue
